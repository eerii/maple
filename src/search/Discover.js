...
const Card = ({show, name, username, language, otherLanguages, funfact, bio, offer}) => {
    return (
        <CardStyle style={{margin: "8px", display: `${show ? "block" : "none"}`, minWidth: "300px"}}>
            <div style={{boxShadow: "0 3px rgba(199,199,217,0.5)", borderRadius: "16px", padding: "20px"}}>
                <h2 style={{margin: "0 0 4px 0"}}>{name}</h2>
                <h4 style={{margin: "0"}}>{`@${username}`}</h4>
            </div>

            <div style={{display: "flex", alignItems: "center", justifyContent: "center", flexWrap: "wrap", marginTop: "16px", height: "78px", overflow: "hidden"}}>
                {times(offer.length, i => <div key={"circle" + i}>
                    <CategoryStyle style={{margin: "4px", cursor: "default"}}
                                   color={colors.filter((c, j) => codes[j] === offer[i])}
                                   active={true}
                                   radius={6}>
                        <p style={{margin: "0", color: "white", fontSize: "12px"}}>{names.filter((n, j) => codes[j] === offer[i])[0].replace(":", " ")}</p>
                    </CategoryStyle>
                </div>)}
            </div>
            ...
            <div style={{textAlign: "left", padding: "0 16px", marginBottom: "16px"}}>
                ...
                {otherLanguages && otherLanguages[0] !== "" && <p style={{margin: "4px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap"}}><b>Other Languages:</b> {otherLanguages.join(", ").split(0, -2)}</p>}
...
const Discover = ({ loggedIn }) => {
                ...
                if (load && loggedIn) {
                    ...
                    if (localUsers !== null) {
                        ...
                        const get = await axios.get(
                            process.env.REACT_APP_URL + "/api/discover", { headers: headers })
    ...
    return (
        <Background style={{paddingTop: "15vh", paddingBottom: "15vh", minHeight: "65vh", paddingLeft: 0, paddingRight: 0}}>
            <div style={{paddingLeft: "10vw", paddingRight: "10vw"}}>
                <h1 style={{marginLeft: "8px", marginBottom: "4px"}}>Discover</h1>
                <div style={{display: "flex", justifyContent: "space-between"}}>
                    <div style={{display: "flex", alignItems: "center"}}>
                        {need !== null && times(need.length + 1, i => <div style={{marginRight: "8px"}} key={"circle" + i}>
                            <CategoryStyle style={{margin: `${4 - ((active === i && selected === -1) || selected === i ? 2 : 0)}px ${4 - ((active === i && selected === -1) || selected === i ? 4 : 0)}px`}}
                                ...
                                <h4 style={{margin: "0", color: "white"}}>{i === need.length ?
                                    "+" :
                                    names.filter((n, j) => codes[j] === need[i])[0].replace(":", " ")}</h4>
                            </CategoryStyle>
                        </div>)}
                    </div>
                    <div style={{display: "flex", alignItems: "center"}}>
            ...
            <div style={{overflow: "hidden", marginTop: "32px"}}>
                <CardSlider>
                    ...
                    {users && times(users.length, i => {
                            const filter = JSON.parse(users[i].offer).filter((o) => {
                                if (selected !== -1) {
                            ...
                            return <Card show={filter} name={users[i].name} username={users[i].username}
                                         key={"card" + users[i].username} offer={JSON.parse(users[i].offer)}
                                         language={users[i].language} otherLanguages={JSON.parse(users[i].otherlanguages.replace("{","[").replace("}","]"))}
                                         funfact={users[i].funfact} bio={users[i].bio}/>
                        }
                    )}
